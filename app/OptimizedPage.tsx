...
export default function OptimizedPage() {
  ...
  return <main className="app-shell"><div className="page-container">
    ...
    {!!points.length && <div className="results animate-fade-up">
      ...
      <section className="day-table"><div className="table-heading"><span className="section-kicker"><Layers3 size={14} /> CONTROL POR JORNADA</span><h2>Distancias y cumplimiento</h2></div><table><thead><tr><th>MT FINAL</th><th>Día</th><th>{planningMode === "titles-only" ? "Titulares / meta ×3" : "Titulares"}</th><th>Suplentes</th><th>Prom. titulares</th><th>Prom. suplentes</th></tr></thead><tbody>{tableRows.map((row) => <tr key={`${row.mt}-${row.day}`}><td>{row.mt}</td><td>Día {row.day}</td><td>{row.tit} / {(forecast?.[row.mt]?.[row.day] ?? 0) * (planningMode === "titles-only" ? 3 : 1)}</td><td>{planningMode === "titles-only" ? "No aplica" : `${row.sup} / ${kind === "Suplente" ? "—" : row.tit * 3}`}</td><td>{row.titleAverage.toLocaleString()} m</td><td>{planningMode === "titles-only" ? "—" : `${row.spareAverage.toLocaleString()} m`}</td></tr>)}</tbody></table></section>
    </div>}
