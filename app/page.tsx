...
export default function Home() {
  ...
  const calculate = () => { try { if (!base || !forecast) throw new Error("Carga la base de puntos y el forecast antes de calcular."); const result = assign(extractPoints(base), forecast); setPoints(result.points); setNotices(result.notices); setSelected(null); } catch (e) { setError(e instanceof Error ? e.message : "No fue posible calcular la asignación."); } };
  ...
  return <main>
    ...
    {!!points.length && <>
      <section className="toolbar"><div className="filters"><label>MT FINAL<select value={mt} onChange={(e) => setMt(e.target.value)}><option value="all">Todos los MT</option>{mts.map((x) => <option key={x} value={x}>{x}</option>)}</select></label><label>Día<select value={day} onChange={(e) => setDay(e.target.value)}><option value="all">Todos los días</option>{[...new Set(points.flatMap((p) => p.day ? [p.day] : []))].sort((a,b)=>a-b).map((x) => <option key={x} value={x}>Día {x}</option>)}</select></label><label>Selección<select value={kind} onChange={(e) => setKind(e.target.value)}><option value="all">Titulares y suplentes</option><option value="Titular">Solo titulares</option><option value="Suplente">Solo suplentes</option></select></label></div><button className="download" onClick={download}>Descargar Excel ↓</button></section>
      ...
      <section className="map-section"><div className="map-heading"><div><p className="eyebrow">MAPA DE PLANIFICACIÓN</p><h2>{mt === "all" ? "Todos los MT" : mt}</h2></div><p><i className="dot title" /> Titular <i className="dot spare" /> Suplente<br /><small>El color identifica el día. Selecciona un punto para moverlo.</small></p></div><GeoMap points={filtered} colors={COLORS} onSelect={setSelected} /></section>
