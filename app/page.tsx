...
import { CalendarDays, Check, ChevronDown, CircleGauge, Database, Download, Layers3, LineChart, MapPinned, Route, Search, SlidersHorizontal, Sparkles, Tags, UploadCloud, UserRound, X } from "lucide-react";
...
export default function Home() {
  ...
  return <main className="app-shell"><div className="page-container">
    ...
    {!!points.length && <div className="results animate-fade-up">
      <div className="results-heading"><div><span className="section-kicker"><Sparkles size={14} /> PLANIFICACIÓN CALCULADA</span><h2>Resumen de la operación</h2><p>{filtered.length.toLocaleString()} puntos visibles de {points.length.toLocaleString()} procesados</p></div><button className="download" onClick={download}><Download size={16} /> Descargar Excel</button></div>
